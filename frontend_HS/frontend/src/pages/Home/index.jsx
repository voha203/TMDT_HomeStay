...
function Home() {
  ...
  // 1. Thêm state cho trang hiện tại
  ...
  // 8. Trải nghiệm tốt hơn: Tự quay về trang 1 khi search hoặc sort đổi giá trị
  useEffect(() => {
    setCurrentPage(1);
    ...
    <div className="bg-white min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
        ...
        <section className="max-w-5xl mx-auto -mt-24 relative z-20 mb-20">
          <div className="bg-white shadow-2xl rounded-3xl p-4 md:p-8 border border-gray-100 flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 w-full">
              <label className="block text-xs font-bold uppercase text-gray-400 mb-1 ml-1">Địa điểm</label>
              <input
                type="text"
            ...
            <div className="flex-1 w-full">
              <label className="block text-xs font-bold uppercase text-gray-400 mb-1 ml-1">Ngày đến</label>
            ...
            <button className="w-full md:w-auto bg-blue-900 hover:bg-blue-800 text-white px-10 py-4 rounded-2xl font-bold transition-all shadow-lg shadow-blue-900/20">
              Tìm kiếm
            </button>
          </div>
        </section>
            ...
            {POPULAR_DESTINATIONS.map((dest, index) => (
              <div key={index} className="group cursor-pointer">
                <div className="relative h-64 rounded-3xl overflow-hidden mb-3">
                  <img src={dest.image} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" alt={dest.name} />
                ...
                {currentHomestays.map((room) => (
                  <Link to={`/homestay/${room.id}`} key={room.id} className="group">
                    <div className="relative rounded-[2rem] overflow-hidden mb-4 shadow-md group-hover:shadow-2xl transition-all duration-500">
                      <img src={room.image} alt={room.title} className="w-full h-72 object-cover group-hover:scale-105 transition duration-500" />
                    ...
                    <div className="px-2">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-lg font-bold">{room.category}</span>
                      </div>
                      ...
                      <p className="text-gray-900 font-black text-xl">
                        {room.price?.toLocaleString()} VNĐ <span className="text-sm text-gray-400 font-normal">/ đêm</span>
                      </p>
                    </div>
                  </Link>
                ))}
        ...
        <section className="mt-32 py-16 bg-blue-950 rounded-[3rem] text-white px-10">
          ...
          <div className="grid md:grid-cols-3 gap-12 text-center">
            <div className="flex flex-col items-center">
              <div className="w-20 h-20 bg-blue-900 rounded-3xl flex items-center justify-center text-4xl mb-6 shadow-xl">🏠</div>
              <h3 className="text-xl font-bold mb-3">Homestay chất lượng</h3>
              <p className="text-blue-200 font-light">Mọi căn hộ đều được kiểm duyệt kỹ lưỡng về chất lượng và tiện nghi.</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-20 h-20 bg-blue-900 rounded-3xl flex items-center justify-center text-4xl mb-6 shadow-xl">💳</div>
              <h3 className="text-xl font-bold mb-3">Thanh toán an toàn</h3>
              <p className="text-blue-200 font-light">Hệ thống thanh toán bảo mật, hỗ trợ nhiều phương thức hiện đại.</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-20 h-20 bg-blue-900 rounded-3xl flex items-center justify-center text-4xl mb-6 shadow-xl">⭐</div>
              <h3 className="text-xl font-bold mb-3">Đánh giá minh bạch</h3>
              <p className="text-blue-200 font-light">Đánh giá từ người dùng thực giúp bạn có cái nhìn khách quan nhất.</p>
            </div>
          </div>
        </section>
