...
function Profile() {
  ...
  // Hàm hiển thị màu sắc cho từng trạng thái đơn hàng
  const renderStatusBadge = (status) => {
    switch (status) {
      case "PENDING":
        return <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full font-semibold text-xs border border-yellow-200">Chờ duyệt</span>;
      case "CONFIRMED":
        return <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full font-semibold text-xs border border-green-200">Đã xác nhận</span>;
      case "CANCELLED":
        return <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full font-semibold text-xs border border-red-200">Đã hủy</span>;
      default:
        return <span className="px-3 py-1 bg-gray-100 text-gray-800 rounded-full font-semibold text-xs">{status}</span>;
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 py-32">
        ...
        {/* Khối bảng danh sách đơn đặt phòng */}
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100">
            <h1 className="text-2xl font-black text-gray-950">Lịch sử đặt phòng của tôi</h1>
            <p className="text-gray-500 text-sm mt-1">Theo dõi danh sách trạng thái và hóa đơn phòng bạn thuê</p>
          </div>
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-gray-600 font-medium">
                  {bookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="p-4 pr-6">{renderStatusBadge(booking.status)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
